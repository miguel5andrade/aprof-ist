...
class Perceptron:
    ...
    def update_weight(self, x_i, y_i):
        ...
        y_pred = self.predict(X=x_i)
        if y_pred != y_i:
            # mistake, update the weights
            # subtract x from the wrong class, add x to the correct class
            self.W[y_pred] = self.W[y_pred] - self.eta * x_i
            self.W[y_i] = self.W[y_i] + self.eta * x_i
